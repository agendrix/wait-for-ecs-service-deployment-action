...
export async function waitForDeploymentOutcome(clusterName: string, serviceName: string, taskDefinitionArn: string, deploymentTimeout: NodeJS.Timeout, statusCheckFrequencyInMs: number = STATUS_CHECK_FREQUENCY_MS) {
  ...
  if (primaryDeployment.taskDefinitionArn !== taskDefinitionArn) {
    ...
    if(deploymentTimeout.hasRef()) waitForDeploymentOutcome(clusterName, serviceName, taskDefinitionArn, deploymentTimeout);
  }
}
